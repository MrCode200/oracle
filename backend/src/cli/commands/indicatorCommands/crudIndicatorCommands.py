...
def update_indicator_command(
    ...
    indicator_id: int = validate_and_prompt_indicator_id(profile_id, indicator_id)
    ...
    while True:
        ...
        match option:
            case 0:
                break
            case 1:
                new_indicator_settings = create_edit_object_settings(obj=indicator.instance,
    ...
    if ticker != indicator.ticker and ticker is not None:
        changes_extra_table.add_row("0", "Ticker", indicator.ticker, ticker)
    else:
        ticker = indicator.ticker

    if interval != indicator.interval and interval is not None:
        changes_extra_table.add_row("1", "Interval", indicator.interval, interval)
    else:
        interval = indicator.interval

    if weight != indicator.weight and weight is not None:
        changes_extra_table.add_row("2", "Weight", str(indicator.weight), str(weight))
    else:
