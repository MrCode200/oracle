...
def create_indicator(profile_id: int, indicator_name: str, indicator_settings: dict
                     ) -> Indicator | None:
    """
...
def get_indicator(profile_id: int = None, indicator_id: int = None,
                  ticker: str = None):
    """
...
def update_indicator(indicator_id: int, indicator_settings: dict):
    ...
    try:
        ...
        logger.error(f"Error updating indicator with ID {indicator_id}: {e}", exc_info=True)
        session.rollback()
        return False
